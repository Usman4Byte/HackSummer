...
function readsForScenario(scenario: Scenario, hasLLM: boolean): ReadSource[] {
  ...
  // Free chat: only surface the fake KB read when we're relying on the
  // deterministic fallback answer (which does reference LRU caching). With a
  // real LLM answering the user's actual question, no external source was
  // consulted — the Input Shield panel will honestly say so.
...
export async function runTurn(
  ...
  // Tool calls: Sentinel-on runs firewall; Sentinel-off allows everything
  ...
  let claims: Claim[];
  if (!sentinelOn) {
    // raw: everything painted "confident" (grounded)
